"""
LoCoMo Recall@5 Benchmark — multi-qa-mpnet-base-dot-v1
-------------------------------------------------------
Full benchmark on locomo10.json with corrected production scoring:
  - multi-qa-mpnet-base-dot-v1 embeddings (retrieval-tuned, question→passage)
  - Decay excluded from ranking (used for pruning only, same as retrieve.py fix)
  - Hybrid score: W_BM25 × bm25_norm + W_VECTOR × cosine
  - Session summaries stored per session with real timestamps

Compared against locomo_mpnet.py baseline (all-mpnet-base-v2 + sim×strength = 59%).

Usage:
    python benchmarks/locomo_qa_model.py
"""
...
TOP_K        = 5
SIM_THRESH   = 0.20   # lower than production 0.50 — summaries are shorter, sims are lower
PRUNE_THRESH = 0.05
IMPORTANCE   = 0.7
W_BM25       = 0.4
W_VECTOR     = 0.6
...
class MemoryStore:
    ...
    def search(self, query: str, query_time: datetime) -> list:
        ...
        bm25_norm = np.array([s / (s + 1.0) if s > 0 else 0.0 for s in raw_bm25])
        ...
        for i, m in enumerate(self.memories):
            ...
            # Decay: pruning signal only — not in ranking formula.
            # Use simulated query_time as reference (not wall-clock now),
            # because LoCoMo sessions are from 2023 and compute_strength
            # uses datetime.now() internally which would make everything ~1000 days old.
            days = max(0, (query_time - m["last_accessed"]).total_seconds() / 86400)
            effective_lambda = 0.16 * (1 - m["importance"] * 0.8)
            strength = m["importance"] * math.exp(-effective_lambda * days) * (1 + m["recall_count"] * 0.2)
            if strength < PRUNE_THRESH:
                continue

            score = W_BM25 * bm25_norm[i] + W_VECTOR * sim
...
def run():
    ...
    for idx, sample in enumerate(data):
        ...
        hits = 0
        for qa in all_qa:
            top = store.search(qa["question"], query_time=query_time)
            if answer_hit(qa["answer"], top):
                hits += 1
    ...
    print("=" * 65)
    print(f"\nYourMemory Recall@5 (multi-qa-mpnet-base-dot-v1): {overall}%")
    print(f"Previous result    (all-mpnet-base-v2):           59%")
